...
export interface FirestoreTask {
  ...
  question: string;
  taskType: 'quiz' | 'flashcard' | 'case' | 'rhythm' | 'lead-match' | 'video'; // Add 'video'
  optionA?: string;
  optionB?: string;
  optionC?: string;
  optionD?: string;
...
const verifyFirebaseConnection = async (isGuestMode: boolean = false): Promise<boolean> => {
  try {
    if (!db) {
...
let cachedTasks: FirestoreTask[] | null = null;
const cacheTimestamp: { time: number } = { time: 0 };
const CACHE_EXPIRY = 30 * 60 * 1000; // 30 minutes in milliseconds

// Fetch all active tasks from Firestore
export const fetchAllActiveTasks = async (isGuestMode: boolean = false): Promise<FirestoreTask[]> => {
  try {
    ...
    if (!isConnected) {
      ...
      const sampleTasks = getSampleTasks();
      console.log('📋 Using sample tasks with video content:', 
        sampleTasks.filter(t => t.taskType === 'video').length);
      return sampleTasks;
    }
    ...
    if (isGuestModeConfirmed && videoTasks.length < 3) {
      ...
      console.log('📺 Video tasks after enhancement:', 
        enhancedTasks.filter(t => t.taskType === 'video').length);
...
const getSampleTasks = (): FirestoreTask[] => {
  return [
    {
      id: 'sample-1',
      taskId: 1,
...
export const updateTaskUsage = async (taskId: string): Promise<void> => {
  try {
...
export const selectDailyTasks = (allTasks: FirestoreTask[], seed: number, count: number = 5): FirestoreTask[] => {
  ...
  const otherTasks = allTasks.filter(task => 
    task.taskType !== 'video' && task.taskType !== 'quiz'
  );
  ...
  const recentlyUnusedTasks = allTasks.filter(task => 
    !task.lastUsedDate || task.lastUsedDate < sevenDaysAgoStr
  );
  console.log('📅 Tasks not used in last 7 days:', recentlyUnusedTasks.length);

  // Initialize selection array
  const selected: FirestoreTask[] = [];
  ...
  const remainingTasksPool = allTasks.filter(task => 
    !selected.some(s => s.id === task.id)
  );
  ...
  selected.forEach(task => {
    const difficulty = task.difficulty as keyof typeof difficultyCount;
    if (difficulty) difficultyCount[difficulty]++;
  });
  ...
  while (selected.length < count && selected.length < allTasks.length) {
    const remaining = weightedTasks.filter(
      task => !selected.some(s => s.id === task.id)
    );
