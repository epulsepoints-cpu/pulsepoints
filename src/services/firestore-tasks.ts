...
export interface FirestoreTask {
  ...
  question: string;
  taskType: 'quiz' | 'flashcard' | 'case' | 'rhythm' | 'lead-match' | 'video';
  optionA?: string;
  optionB?: string;
  optionC?: string;
  optionD?: string;
  ...
  // Image URL for ECG tasks (support both property names for compatibility)
  ecgImageUrl?: string;
  imageUrl?: string;
...
const getCachedTasks = (): FirestoreTask[] | null => {
  try {
    ...
    if (cached) {
      const parsedCache: TaskCache = JSON.parse(cached);
...
const setCachedTasks = (tasks: FirestoreTask[]) => {
  try {
...
const getEnhancedSampleTasks = (): FirestoreTask[] => {
  return [
    // Video Tasks
    {
      id: 'sample-video-1',
...
export const selectDailyTasks = (
  allTasks: FirestoreTask[], 
  seed: number, 
  count: number = 5,
  prioritizeVideos: boolean = false
): FirestoreTask[] => {
  ...
  let selectedTasks: FirestoreTask[] = [];
  ...
  if (remainingSlots > 0 && otherTasks.length > 0) {
    const availableOtherTasks = otherTasks.filter(
