...
export const getCurrentActiveEvent = async (): Promise<WeeklyEvent | null> => {
  try {
    const now = new Date();
    ...
    if (!snapshot.empty) {
      const eventData = snapshot.docs[0].data() as WeeklyEvent;
      return {
        ...eventData,
...
// Get upcoming events
export const getUpcomingEvents = async (): Promise<WeeklyEvent[]> => {
  try {
    const now = new Date();
    const eventsRef = collection(db, COLLECTIONS.EVENTS);
    const q = query(
    ...
    return snapshot.docs.map(doc => {
      const eventData = doc.data() as WeeklyEvent;
      return {
        ...eventData,
        startDate: eventData.startDate instanceof Timestamp ? eventData.startDate.toDate() : new Date(eventData.startDate),
        endDate: eventData.endDate instanceof Timestamp ? eventData.endDate.toDate() : new Date(eventData.endDate)
...
export const createWeeklyEvent = async (eventData: Omit<WeeklyEvent, 'id'>): Promise<string> => {
  try {
    const eventsRef = collection(db, COLLECTIONS.EVENTS);
...
export const updateEventStatus = async (eventId: string, status: WeeklyEvent['status']): Promise<void> => {
  try {
...
export const getUserEventProgress = async (userId: string, eventId: string): Promise<UserEventProgress | null> => {
  try {
    const progressRef = doc(db, COLLECTIONS.USER_PROGRESS, `${userId}_${eventId}`);
    const progressDoc = await getDoc(progressRef);
    
    if (progressDoc.exists()) {
      const data = progressDoc.data() as UserEventProgress;
      return {
        ...data,
...
export const initializeUserEventProgress = async (userId: string, eventId: string): Promise<void> => {
  try {
    const progressRef = doc(db, COLLECTIONS.USER_PROGRESS, `${userId}_${eventId}`);
    
    const initialProgress: UserEventProgress = {
      userId,
      eventId,
      daysCompleted: [],
      totalXpEarned: 0,
...
export const completeDayProgress = async (
  userId: string, 
  eventId: string, 
  dayNumber: number,
  xpEarned: number,
  ...
  try {
    ...
    const currentProgress = progressDoc.data() as UserEventProgress;
...
export const getEventConfig = async (): Promise<EventConfig | null> => {
  try {
    ...
    if (configDoc.exists()) {
      return configDoc.data() as EventConfig;
    }
...
// Get event templates
export const getEventTemplates = async (): Promise<WeeklyEventTemplate[]> => {
  try {
    const templatesRef = collection(db, COLLECTIONS.EVENT_TEMPLATES);
    const snapshot = await getDocs(templatesRef);
    
    return snapshot.docs.map(doc => doc.data() as WeeklyEventTemplate);
  } catch (error) {
...
export const subscribeToCurrentEvent = (callback: (event: WeeklyEvent | null) => void) => {
  const now = new Date();
  const eventsRef = collection(db, COLLECTIONS.EVENTS);
  const q = query(
    eventsRef,
...
export const subscribeToUserProgress = (
  userId: string, 
  eventId: string, 
  callback: (progress: UserEventProgress | null) => void
) => {
  const progressRef = doc(db, COLLECTIONS.USER_PROGRESS, `${userId}_${eventId}`);
  
  return onSnapshot(progressRef, (doc) => {
    if (doc.exists()) {
      const data = doc.data() as UserEventProgress;
      const progress = {
        ...data,
        startedAt: data.startedAt instanceof Timestamp ? data.startedAt.toDate() : new Date(data.startedAt),
        lastActivityAt: data.lastActivityAt instanceof Timestamp ? data.lastActivityAt.toDate() : new Date(data.lastActivityAt),
...
export const generateNextWeekEvent = async (): Promise<string | null> => {
  try {
    ...
    if (!lastEventSnapshot.empty) {
      const lastEvent = lastEventSnapshot.docs[0].data() as WeeklyEvent;
    ...
    const newEvent: Omit<WeeklyEvent, 'id'> = {
