...
export const getUserProgressStats = async (userId) => {
    try {
        ...
        const progressQuery = query(collection(db, 'moduleProgress'), where('userId', '==', userId));
            ...
            if (progress.lastAccessedAt) {
                try {
                    if (typeof progress.lastAccessedAt === 'string') {
                        ...
                        const timestamp = progress.lastAccessedAt;
 ...
 */
export const updateUserProgressOnLessonComplete = async (userId, lessonData) => {
    try {
        ...
        if (lessonData.score >= 95)
            xpReward += 10; // Perfect score bonus
        if (lessonData.timeSpent < 180)
            xpReward += 5; // Speed bonus
        if (lessonData.passed)
            xpReward += 5; // Pass bonus
 ...
 */
export const initializeUserProgress = async (userId, email) => {
    try {
