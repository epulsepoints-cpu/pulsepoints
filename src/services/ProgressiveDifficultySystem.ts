...
export interface DifficultySettings {
  ...
  complexQuestions: boolean;
  advancedCases: boolean;
  description: string;
}

export const DIFFICULTY_PROGRESSION: DifficultySettings[] = [
  {
    rank: 'ECGKid Intern',
...
export class ProgressiveDifficultySystem {
  static getDifficultySettings(rank: Rank): DifficultySettings {
    return DIFFICULTY_PROGRESSION.find(setting => setting.rank === rank) || DIFFICULTY_PROGRESSION[0];
  }

  static adjustTaskDifficulty(baseTask: any, rank: Rank) {
  ...
  static calculateDifficultyLevel(rank: Rank): 'easy' | 'medium' | 'hard' | 'expert' {
    const settings = this.getDifficultySettings(rank);
    
    if (settings.difficultyMultiplier <= 0.8) return 'easy';
    if (settings.difficultyMultiplier <= 1.1) return 'medium';
    if (settings.difficultyMultiplier <= 1.5) return 'hard';
    return 'expert';
  }

  static getProgressionInsights(currentRank: Rank, nextRank?: Rank) {
  ...
  static getRecommendedStudyPlan(rank: Rank): string[] {
    const settings = this.getDifficultySettings(rank);
