...
export interface Achievement {
  ...
  title: string;
  description: string;
  type: 'xp' | 'streak' | 'tasks' | 'video' | 'quiz' | 'flashcard' | 'special';
  requirement: number;
  reward: {
    xp?: number;
    gems?: number;
...
export const checkAchievements = async (userId: string, user: User): Promise<Achievement[]> => {
...
const awardAchievementRewards = async (userId: string, achievements: Achievement[]) => {
  try {
    ...
    if (userDoc.exists()) {
      ...
      achievements.forEach(achievement => {
        if (achievement.reward.xp) totalXpReward += achievement.reward.xp;
        if (achievement.reward.gems) totalGemReward += achievement.reward.gems;
      });
...
export const updateUserActivity = async (userId: string, activity: {
  type: 'quiz' | 'video' | 'flashcard' | 'task';
  taskId?: string;
  points?: number;
  correct?: boolean;
}) => {
