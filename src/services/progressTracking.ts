...
export interface UserProgress {
  userId: string;
  currentEvent: number; // Changed from currentWeek to currentEvent for FIFA style
  completedEvents: string[];
  eventProgress: {
    [eventId: string]: {
      currentDay: number; // Track current unlocked day
  ...
  eventStats: { // Changed from weeklyStats to eventStats
    [eventId: string]: {
      totalXP: number;
      totalECGCoins: number; // Changed from gems to ECGCoins
      badges: string[];
      averageScore: number;
  ...
  achievements: {
    ...
    description: string;
    unlockedAt: Date;
    category: 'completion' | 'score' | 'speed' | 'streak' | 'special';
  }[];
}

// FIFA Mobile-style reward system interface
export interface RewardData {
  xp: number;
  ecgCoins: number; // Changed from gems to ECGCoins
  badges: string[];
  achievements: string[];
...
export const calculateEventRewards = (eventId: string, dayId: string, score: number): RewardData => {
  const eventRewards = {
  ...
  const eventReward = eventRewards[eventId as keyof typeof eventRewards] || { baseXP: 100, baseCoins: 25 };
 ...
 */
const calculateAverageScore = (progress: UserProgress, eventId: string): number => {
  const eventProgress = progress.eventProgress[eventId];
  if (!eventProgress || eventProgress.completedTasks.length === 0) return 0;
 ...
 */
const checkAndAwardAchievements = async (userId: string, progress: UserProgress): Promise<void> => {
  ...
  // Speed demon
  const fastCompletions = Object.values(progress.taskScores).filter(task => 
    task.completed && task.timeSpent < 300 // Under 5 minutes
  ).length;
 ...
 */
export const getEventLeaderboard = async (eventId: string): Promise<any[]> => {
  try {
