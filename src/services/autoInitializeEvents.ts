  ...
  try {
    ...
    const events = [
      // EVENT 1: ECG CORE FOUNDATIONS (30 Days)
      {
        ...
        totalRewards: {
          xp: 15000, // 30 days × 500 XP average (5 tasks per day)
          ecgCoins: 3000,
 ...
 * Each day has 6 progressive tasks (loops)
 */
    ...
    tasks: [
      {
        ...
        type: 'ecg-rhythm',
        title: 'What is ECG?',
        description: 'Introduction to ECG rhythm analysis',
        content: {
          stripImage: '/images/ecg-strips/normal_sinus_intro.png',
          stripImageAlt: '/images/ecg-strips/alt_normal_sinus_intro.png',
 ...
 */
function generateDayTasks(dayNumber: number, theme: string) {
  const taskTypes = ['ecg-rhythm', 'h5p-quiz', 'image-hotspot', 'drag-drop', 'ecg-simulator'];
  const tasks = [];
 ...
 */
function generateTaskContent(taskType: string, theme: string, day: number, taskNumber: number) {
  const baseContent = {
 ...
 */
function getRandomRhythm(theme: string): string {
  const rhythmMap = {
  ...
  const rhythms = rhythmMap[theme as keyof typeof rhythmMap] || rhythmMap.foundations;
