...
export const generateEventDays = (eventId) => {
    const eventConfigs = {
    ...
    const config = eventConfigs[eventId] || eventConfigs['shock-zone-challenge'];
...
const getDifficultyForDay = (dayNumber) => {
    if (dayNumber <= 7)
        return 'easy';
    if (dayNumber <= 14)
        return 'medium';
...
const generateDayTasks = (eventId, dayNumber, topic, difficulty) => {
...
const getTaskTitle = (taskType, taskNumber) => {
    const titles = {
    ...
    return titles[taskType] || `Task ${taskNumber}`;
};
const getTaskDescription = (taskType, topic) => {
    const descriptions = {
    ...
    return descriptions[taskType] || `Practice ${topic.toLowerCase()}`;
};
const getEstimatedTime = (taskType, difficulty) => {
    const baseTimes = {
        'h5p-quiz': 5,
        'ecg-rhythm': 7,
        'image-hotspot': 6,
    ...
    const baseTime = baseTimes[taskType] || 8;
    const multiplier = difficultyMultiplier[difficulty] || 1;
    return Math.round(baseTime * multiplier);
};
const getTaskXP = (taskType, difficulty) => {
    const baseXP = {
        'h5p-quiz': 25,
        'ecg-rhythm': 35,
        'image-hotspot': 30,
    ...
    const base = baseXP[taskType] || 30;
    const multiplier = difficultyMultiplier[difficulty] || 1;
    return Math.round(base * multiplier);
};
const getTaskGems = (taskType, difficulty) => {
    const baseGems = {
        'h5p-quiz': 3,
        'ecg-rhythm': 5,
        'image-hotspot': 4,
    ...
    const base = baseGems[taskType] || 5;
    const multiplier = difficultyMultiplier[difficulty] || 1;
    return Math.round(base * multiplier);
};
// Generate quiz points for interactive video based on topic
const generateQuizPointsForTopic = (topic, videoDuration) => {
    ...
    const topicKey = Object.keys(commonQuestions).find(key => topic.toLowerCase().includes(key.toLowerCase()));
...
const generateQuizQuestions = (topic, difficulty) => {
    ...
    const diffQuestions = topicQuestions[difficulty];
    return diffQuestions || topicQuestions.easy || [];
};
const generateECGQuestions = (topic, difficulty) => {
...
const generateHotspots = (topic, difficulty) => {
    const hotspotBank = {
        'VTach Recognition': [
            {
                x: 25,
...
const generateDragItems = (topic, difficulty) => {
    const dragItemBank = {
        'VTach Recognition': [
...
const generatePatientInfo = (topic, difficulty) => {
    ...
    const scenarios = patientScenarios[topic];
    if (scenarios) {
        const diffScenario = scenarios[difficulty];
        return diffScenario || scenarios.easy || scenarios[Object.keys(scenarios)[0]];
    }
    return {
        age: 65,
        gender: 'male',
...
export const getEventDays = (eventId) => {
    return generateEventDays(eventId);
};
export const getTaskById = (eventId, taskId) => {
