 ...
 */
export function useTaskReminders(checkInterval = 86400000) {
    const { user } = useAuth();
    useEffect(() => {
        if (!user?.uid)
            return;
        const checkIncompleteItems = async () => {
            try {
                ...
                const q = query(incompleteRef, where('userId', '==', user.uid), where('started', '==', true), where('completed', '==', false), where('lastUpdated', '<', Timestamp.fromDate(new Date(Date.now() - 172800000))) // 2 days ago
                );
                ...
                for (const doc of snapshot.docs) {
                    ...
                    const reminderQuery = query(reminderRef, where('userId', '==', user.uid), where('type', '==', 'reminder'), where('relatedItemId', '==', lessonId), where('timestamp', '>', Timestamp.fromDate(new Date(Date.now() - 172800000))) // Last 2 days
                    );
                ...
                const tasksQuery = query(tasksRef, where('userId', '==', user.uid), where('completed', '==', false), where('dueDate', '<', Timestamp.now()), orderBy('dueDate', 'asc'), limit(5) // Limit to avoid too many notifications
                );
                const tasksSnapshot = await getDocs(tasksQuery);
                for (const doc of tasksSnapshot.docs) {
                    const task = doc.data();
                    // Check if we already sent a reminder for this task recently
                    const taskReminderQuery = query(collection(db, 'userNotifications'), where('userId', '==', user.uid), where('type', '==', 'reminder'), where('relatedItemId', '==', task.id), where('timestamp', '>', Timestamp.fromDate(new Date(Date.now() - 86400000))) // Last 24 hours
                    );
                ...
                if (!userSnapshot.empty) {
                    ...
                    if (hoursSinceActive >= 20 && hoursSinceActive < 24) {
                        ...
                        const streakReminderQuery = query(collection(db, 'userNotifications'), where('userId', '==', user.uid), where('type', '==', 'reminder'), where('title', '==', '🔥 Maintain Your Streak!'), where('timestamp', '>', Timestamp.fromDate(new Date(Date.now() - 86400000))) // Last 24 hours
                        );
