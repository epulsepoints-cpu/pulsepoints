...
import { RANK_THRESHOLDS } from '@/types/game';
import { toast } from '@/components/ui/use-toast';
import { fetchAllActiveTasks, updateTaskUsage } from '@/services/firestore-tasks';
...
const validTaskTypes = ['quiz', 'flashcard', 'case', 'rhythm', 'lead-match', 'video'];
const validDifficulties = ['easy', 'medium', 'hard'];
const isValidTaskType = (type) => {
    return validTaskTypes.includes(type);
};
...
const selectDailyTasksWithVideos = (allTasks, seed, count = 5) => {
...
export const useGameState = () => {
    ...
    const fetchDailyTasksFromFirestore = useCallback(async () => {
        try {
            ...
            const processedTasks = selectedTasks
                .map((firestoreTask) => {
                // Validate and sanitize task type
                const rawTaskType = firestoreTask.taskType || 'quiz';
                const taskType = isValidTaskType(rawTaskType) ? rawTaskType : 'quiz';
                // Validate and sanitize difficulty
                const rawDifficulty = firestoreTask.difficulty || 'easy';
                const difficulty = isValidDifficulty(rawDifficulty) ? rawDifficulty : 'easy';
                const optionsArray = [
    ...
    const updateUserStreaks = useCallback(async (userId) => {
        try {
            const userRef = doc(db, "users", userId);
            const userSnap = await getDoc(userRef);
            if (userSnap.exists()) {
    ...
    const completeTask = useCallback(async (taskId, isCorrect) => {
        ...
        setGameState(prev => {
            ...
            const updatedTasks = prev.dailyTasks.map(t => t.id === taskId ? { ...t, completed: true, earnedPoints: points } : t);
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            window.debugGameState = debugCurrentState;
        }
    }, [debugCurrentState]);
    ...
    const completeLearningLesson = useCallback(async (moduleId, lessonId, score, timeSpent, perfect = false) => {
            ...
            setGameState(prev => {
                ...
                const updatedProgress = {
                    ...
                    [moduleId]: {
                        ...
                        status: newCompletedLessons >= currentModule.totalLessons ? 'completed' : 'in-progress'
                    }
                };
        ...
        try {
            ...
            try {
                ...
                const progressionResult = await handleLessonProgression(userId, moduleId, lessonId, score, timeSpent, true // First attempt
                );
                ...
                setGameState(prev => {
                    ...
                    const updatedProgress = {
                        ...
                        [moduleId]: {
                            ...
                            streak: perfect ? (currentModule.streak || 0) + 1 : 0,
                            masteryLevel: Math.min(100, (currentModule.masteryLevel || 0) + (perfect ? 5 : 3)),
                            status: currentModule.completedLessons + 1 >= currentModule.totalLessons ? 'completed' : 'in-progress'
                        }
                    };
                    // Handle hearts: restore for perfect, lose for poor performance
                    let heartsChange = 0;
        ...
        if (gameState.isGuestUser) {
            ...
            if (currentHearts <= 0) {
                toast({
                    ...
                    description: 'Hearts will regenerate over time. Take a break and come back later!',
                    variant: 'destructive',
                });
            }
            else {
    ...
    const regenerateHearts = useCallback(async () => {
        if (!gameState.user || gameState.user.hearts >= 5)
            return;
        if (gameState.isGuestUser) {
    ...
    const updateLearningProgressInState = useCallback((moduleId, progressData) => {
        setGameState(prev => ({
            ...prev,
            learningProgress: {
                ...prev.learningProgress,
