...
module.exports = (req, res) => {
  ...
  // List all lessons
  if (action === 'list') {
    const lessonsMetadata = Object.keys(lessonDatabase).map(key => {
      const lesson = lessonDatabase[key];
      return {
        id: lesson.id,
  ...
  return res.status(200).json({
    ...
    usage: {
      listLessons: '/api/index?action=list',
      getLesson: '/api/index?lesson=lesson-id',
      info: '/api/index'
    },
    availableLessons: Object.keys(lessonDatabase),
    endpoints: [
      'GET /api/index - this info',
      'GET /api/index?action=list - get all lessons metadata',
      'GET /api/index?lesson=lesson-id - get specific lesson content'
    ]
  });
};