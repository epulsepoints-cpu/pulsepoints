...
module.exports = (req, res) => {
  ...
  // Health check
  if (url.includes('/health') || url.includes('/test') || url === '/api/app') {
    return res.status(200).json({
      status: 'healthy',
      message: 'PulsePoints Lessons API is running',
      timestamp: new Date().toISOString(),
      endpoints: [
        'GET /api/app - this endpoint',
        'GET /api/app?lessons - get all lessons',
        'GET /api/app?lesson=lesson-id - get specific lesson'
      ]
    });
  }

  const urlParams = new URL('http://dummy.com' + url);
  const lessonId = urlParams.searchParams.get('lesson');
  const wantLessons = urlParams.searchParams.has('lessons');
  ...
  // Default response
  return res.status(200).json({
    message: 'PulsePoints Lessons API',
    usage: {
      lessons: '/api/app?lessons',
      lesson: '/api/app?lesson=lesson-id',
      health: '/api/app'
    },
    availableLessons: Object.keys(lessonDatabase)
  });
